...
pub mod region_operations_client {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + Send,
    {
        ...
        #[doc = ""]
        #[doc = " This method is called on a best-effort basis. Specifically:"]
        #[doc = " - In uncommon cases, when the server is overloaded, the request might return before the default deadline is reached, or might return after zero seconds."]
        #[doc = " - If the default deadline is reached, there is no guarantee that the operation is actually done when the method returns. Be prepared to retry if the operation is not `DONE`."]
        pub async fn wait(
            &mut self,
